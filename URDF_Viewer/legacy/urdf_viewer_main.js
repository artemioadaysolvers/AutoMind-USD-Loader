...
function createMechanismDecorationController(core, robot, urdfText = '') {
  ...
  group.name = 'automind_mechanism_decorations';
  group.visible = true;
  ...
  function disposeObj(o) {
    try { o?.traverse?.(x => { x.geometry?.dispose?.(); if (Array.isArray(x.material)) x.material.forEach(m => m?.dispose?.()); else x.material?.dispose?.(); }); } catch (_) {}
  }
  function clear() {
    axisItems.splice(0).forEach(it => { try { group.remove(it.arrow); disposeObj(it.arrow); } catch (_) {} });
    loopItems.splice(0).forEach(it => { try { group.remove(it.line); it.line.geometry?.dispose?.(); it.line.material?.dispose?.(); } catch (_) {} });
  }
  ...
  function isDrawableJoint(j) {
    ...
    return t.includes('revolute') || t.includes('continuous') || t.includes('prismatic') || t.includes('floating') || t.includes('planar');
  }
  function rebuild() {
    clear();
    const dim = modelDim();
    const len = Math.max(dim * 0.18, 1e-4);
    if (showAxes) {
      Object.values(robot?.joints || {}).forEach(j => {
        if (!isDrawableJoint(j)) return;
        const t = String(j.jointType || j.type || '').toLowerCase();
        const arrow = new THREE.ArrowHelper(jointWorldAxis(j), jointWorldPivot(j), len, t.includes('prismatic') ? 0x14b8b9 : 0xffcc00, len * 0.22, len * 0.08);
        arrow.name = `joint_axis:${j.name || ''}`;
        arrow.renderOrder = 9998;
        group.add(arrow);
        axisItems.push({ arrow, joint: j, len, head: len * 0.22, width: len * 0.08 });
      });
    }
    if (showLoops && loops.length) {
      for (const lp of loops) {
        const la = robot?.links?.[lp.a];
        const lb = robot?.links?.[lp.b];
        if (!la || !lb) continue;
        const pa = la.getWorldPosition(new THREE.Vector3());
        const pb = lb.getWorldPosition(new THREE.Vector3());
        const geom = new THREE.BufferGeometry().setFromPoints([pa, pb]);
        const mat = new THREE.LineBasicMaterial({ color: 0xff5b7f, transparent: true, opacity: 0.95, depthTest: false, depthWrite: false });
        const line = new THREE.Line(geom, mat);
        line.name = `loop:${lp.a}<->${lp.b}`;
        line.renderOrder = 9997;
        group.add(line);
        loopItems.push({ line, a: la, b: lb });
      }
    }
  }
  function update() {
    if (!showAxes && !showLoops) return;
    for (const it of axisItems) {
      try { it.arrow.position.copy(jointWorldPivot(it.joint)); it.arrow.setDirection(jointWorldAxis(it.joint)); it.arrow.setLength(it.len, it.head, it.width); } catch (_) {}
    }
    for (const it of loopItems) {
      try {
        const arr = it.line.geometry.attributes.position.array;
        const pa = it.a.getWorldPosition(new THREE.Vector3());
        const pb = it.b.getWorldPosition(new THREE.Vector3());
  ...
  function destroy() { clear(); try { group.parent?.remove(group); } catch (_) {} }
