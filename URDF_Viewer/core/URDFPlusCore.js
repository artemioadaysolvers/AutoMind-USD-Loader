...
// AutoMind URDF+ loader for the modular USD-style viewer shell. V10 texture-safe merge.
...
class URDFPlusAssetResolver {
  ...
  _lookupMap(map, path) {
    if (!path) return '';
    if (map === this.byKey && /^(blob:|data:|https?:\/\/)/i.test(String(path))) return String(path);
