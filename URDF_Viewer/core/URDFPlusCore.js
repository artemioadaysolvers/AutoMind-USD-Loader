...
const AUTOMIND_TEXTURE_PIPELINE = {
  ...
  // once.
  DAE_INLINE_SETTLE_MS: 15,
  DAE_PARSE_SETTLE_MS: 24,
  DAE_TEXTURE_TIMEOUT_MS: 4667,
  DAE_POST_TEXTURE_SETTLE_MS: 47,
  BETWEEN_DAE_LOADS_MS: 18,
};
// BUILD159: same deterministic sequencing as BUILD158, but waits/timeouts are 3x shorter.
...
async function waitForObjectTexturesReady(obj, timeoutMs = AUTOMIND_TEXTURE_PIPELINE.DAE_TEXTURE_TIMEOUT_MS) {
  ...
  const per = Math.max(240, Math.floor(timeoutMs / Math.max(1, Math.min(4, maps.length))));
...
async function loadMeshCandidate(candidate, resolver, fallbackMaterial) {
  ...
  if (ext === '.dae') {
    ...
    await waitForObjectTexturesReady(obj, Math.max(1400, AUTOMIND_TEXTURE_PIPELINE.DAE_TEXTURE_TIMEOUT_MS / 2));
