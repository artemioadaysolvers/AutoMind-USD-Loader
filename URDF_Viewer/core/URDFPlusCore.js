...
// AutoMind URDF+ loader for the modular USD-style viewer shell. BUILD158: deterministic paced DAE base64 texture pipeline + queued thumbnails.
...
const AUTOMIND_TEXTURE_PIPELINE = {
  ...
  // once.
  DAE_INLINE_SETTLE_MS: 45,
  DAE_PARSE_SETTLE_MS: 70,
  DAE_TEXTURE_TIMEOUT_MS: 14000,
  DAE_POST_TEXTURE_SETTLE_MS: 140,
  BETWEEN_DAE_LOADS_MS: 55,
};
...
async function waitForObjectTexturesReady(obj, timeoutMs = AUTOMIND_TEXTURE_PIPELINE.DAE_TEXTURE_TIMEOUT_MS) {
  ...
  const per = Math.max(700, Math.floor(timeoutMs / Math.max(1, Math.min(4, maps.length))));
...
async function loadMeshCandidate(candidate, resolver, fallbackMaterial) {
  ...
  if (ext === '.dae') {
    ...
    await waitForObjectTexturesReady(obj, Math.max(4000, AUTOMIND_TEXTURE_PIPELINE.DAE_TEXTURE_TIMEOUT_MS / 2));
