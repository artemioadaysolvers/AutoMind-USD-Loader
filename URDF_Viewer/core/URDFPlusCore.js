...
// AutoMind URDF+ loader for the modular USD-style viewer shell. V11 visual-tree + texture URL resolver merge.
  ...
  try {
    if (ext === '.dae') {
      ...
      mgr.setURLModifier((u) => {
        const hit = resolver.resolve(u) || resolver.resolve(cleanPath(u)) || resolver.resolve(basename(u));
        if (hit) return hit;
        const cleaned = cleanPath(u || '');
        const hit2 = resolver.resolve(cleaned) || resolver.resolve(basename(cleaned));
        if (hit2) return hit2;
        // Prevent invalid browser requests such as data:model/base.jpg when a
        // texture is truly missing. Geometry remains visible through material
        // sanitization below.
        if (/\.(png|jpe?g|webp|bmp|gif|tga)$/i.test(cleaned || String(u || ''))) return TRANSPARENT_PNG_DATA_URL;
        return cleaned || u;
      });
...
export async function loadURDFPlusModel(opts = {}) {
  ...
  const joints = Array.from(visualRobotNode.children).filter(n => localName(n) === 'joint').map(n => parseJoint(n, model)).filter(j => j.parent && j.child && model.links[j.parent] && model.links[j.child]);
