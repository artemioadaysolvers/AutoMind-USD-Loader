...
// BUILD152: UNIFIED URDF+ entrypoint. No standard-vs-plus split at runtime.
...
function debug(...args) {
  try { if (globalThis.AutoMindURDFPlusDebug || globalThis.AUTOMIND_DEBUG) console.log('[URDFPLUS_BUILD152_UNIFIED]', ...args); } catch (_) {}
}
...
function renderUnifiedAsyncFacade(opts = {}) {
  ...
  const ready = (async () => {
    ...
    try { realApp.build = 'BUILD152_UnifiedURDFPlusJointSimTextureThumbsFix'; } catch (_) {}
    return realApp;
  })().catch((err) => {
    try {
      if (container) {
