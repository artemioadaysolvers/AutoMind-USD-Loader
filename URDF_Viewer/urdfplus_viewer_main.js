...
function debug(...args) {
    try {
        if (globalThis.AutoMindURDFPlusDebug || globalThis.AUTOMIND_DEBUG)
            console.log('[URDFPLUS_BUILD152_UNIFIED]', ...args);
    }
    catch (_) { }
}
...
function normalizeUnifiedOpts(opts = {}) {
    ...
    return {
        ...
        modelFormat: 'URDF+',
        isURDFPlus: true,
        build: 'BUILD152_UnifiedURDFPlusJointSimTextureThumbsFix',
...
function renderUnifiedAsyncFacade(opts = {}) {
    ...
    const ready = (async () => {
        ...
        if (destroyed)
            return null;
        try {
            container?.querySelector?.('[data-automind-unified-loading="1"]')?.remove?.();
        }
        catch (_) { }
        ...
        try {
            realApp.build = 'BUILD152_UnifiedURDFPlusJointSimTextureThumbsFix';
        }
        catch (_) { }
        return realApp;
    })().catch((err) => {
    ...
    return {
        ...
        resize(...args) { try {
            return realApp?.resize?.(...args);
        }
        catch (_) { } },
        destroy() { destroyed = true; try {
            realApp?.destroy?.();
        }
        catch (_) { } }
    };
