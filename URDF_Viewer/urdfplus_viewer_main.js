...
// BUILD151: UNIFIED URDF+ entrypoint. No standard-vs-plus split at runtime.
...
function debug(...args) {
  try { if (globalThis.AutoMindURDFPlusDebug || globalThis.AUTOMIND_DEBUG) console.log('[URDFPLUS_BUILD151_UNIFIED]', ...args); } catch (_) {}
}
...
function normalizeUnifiedOpts(opts = {}) {
  ...
  return {
    ...
    modelFormat: 'URDF+',
    isURDFPlus: true,
    build: 'BUILD151_UnifiedURDFPlusFullExtrasFix',
...
function renderUnifiedAsyncFacade(opts = {}) {
  ...
  const ready = (async () => {
    ...
    try { realApp.build = 'BUILD151_UnifiedURDFPlusFullExtrasFix'; } catch (_) {}
    return realApp;
  })().catch((err) => {
    try {
      if (container) {
