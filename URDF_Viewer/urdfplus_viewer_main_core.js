...
// BUILD159: URDF+ viewer with 3x faster paced texture decode and queued thumbnails.
...
// BUILD159: thumbnail waits are 3x shorter than BUILD158 while preserving the serial queue.
...
function buildThumbnailer(core, assetToMeshes) {
  ...
  async function waitForTextures(meshes, timeoutMs = 6000) {
    ...
    await Promise.all(maps.map(async (tex) => {
      ...
      if (img && typeof img.decode === 'function') {
        try { await Promise.race([img.decode(), sleep(Math.min(1667, timeoutMs))]); } catch (_) {}
      }
    }));
  ...
  async function primeAll(keys) { for (const k of keys || []) { try { await thumbnail(k); await sleep(40); } catch (_) {} } }
...
export function render(opts = {}) {
  ...
  app.ready = (async () => {
    try {
      ...
      setTimeout(() => { try { thumbs.primeAll(Array.from(assetToMeshes.keys())); } catch (_) {} }, 800);
