...
// BUILD169: clean URDF+ viewer, USD-exact camera presets + uniform shadow/grid.
...
function tweenCamera(core, endPos, endTarget = null, duration = 700) {
  // BUILD169: exact USD viewer camera tween. Do not pre-fit, do not rebuild the
  // start pose, and do not interpolate in a custom orbital path. Capture the
  // camera.position and controls.target exactly as they are visible at click time,
  // then linearly tween both, matching USD_Viewer/ui/ToolsDock.js.
  ...
  function step(t) {
    ...
    if (moveTarget && ctrl?.target) ctrl.target.set(
...
function viewPreset(core, kind = 'iso', object = core.robot, duration = 750) {
  // BUILD169: exact USD viewer viewEndPose logic adapted to URDF+.
