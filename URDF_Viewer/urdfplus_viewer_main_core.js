...
// BUILD158: URDF+ viewer with paced texture decode and serialized thumbnail capture for Colab.
...
function buildThumbnailer(core, assetToMeshes) {
  ...
  async function waitForTextures(meshes, timeoutMs = 18000) {
    ...
    await Promise.all(maps.map(async (tex) => {
      ...
      if (img && typeof img.decode === 'function') {
        try { await Promise.race([img.decode(), sleep(Math.min(5000, timeoutMs))]); } catch (_) {}
      }
    }));
  ...
  async function primeAll(keys) { for (const k of keys || []) { try { await thumbnail(k); await sleep(120); } catch (_) {} } }
...
export function render(opts = {}) {
  ...
  app.ready = (async () => {
    try {
      ...
      setTimeout(() => { try { thumbs.primeAll(Array.from(assetToMeshes.keys())); } catch (_) {} }, 2400);
