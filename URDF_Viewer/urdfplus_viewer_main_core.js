...
// BUILD165: clean URDF+ viewer, 3x faster texture pacing, tween focus, fade Show all restore.
...
function animateMeshVisibility(core, meshes, shouldBeVisible, duration = 540, after = null) {
  ...
  for (const mesh of uniqueMeshes) {
    ...
    for (const mat of mats) {
      ...
      const startOpacity = wasMeshVisible ? currentOpacityFor(mat) : 0;
...
function showAll(core) {
  ...
  // BUILD165: Show all fades hidden components back to the active canonical
  // render mode. It does not change render mode or pass through Ghost/X-Ray;
  // finalization restores the exact current mode.
  const meshes = collectRobotMeshes(core.robot);
