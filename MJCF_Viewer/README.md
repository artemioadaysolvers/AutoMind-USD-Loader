...
## BUILD175 — black-model correction

- Collision geoms declared through `class="collision"` are now excluded before scene creation.
- AutoMind MJCF visual OBJ meshes use a stable texture-first unlit material, avoiding black output from missing or non-uniform CAD normals.
- Removed BUILD174's global `uniform3fv` replacement and per-frame material `needsUpdate`; both were over-broad and could invalidate normal texture programs.
- The fixed world grid/ground matrices from BUILD174 remain unchanged.
- New viewers start in `Solid`; Ghost remains a user-selectable render mode.
