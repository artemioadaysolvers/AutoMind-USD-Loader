...
## BUILD185 vertex-color texture bake

For Google Colab default-framebuffer texture failures, the visible viewport uses CPU-baked UV vertex colors. Thumbnails and MTL decoding remain unchanged.


## BUILD186 — site-pair closed loops and selection repair

- Native support for MuJoCo `equality/connect site1="..." site2="..."` closures.
  The viewer now retains both local site frames rather than silently dropping
  site-pair constraints.
- The axis-point companion of a two-anchor revolute loop remains solver-active
  but is not drawn as a duplicate closure line.
- Selecting a kinematic body no longer includes all nested child-body meshes,
  preventing a large cyan Box3Helper cage around articulated mechanisms.
- Completing an actual joint drag clears the selection cage automatically.
