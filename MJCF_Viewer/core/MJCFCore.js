...
// AutoMind BUILD186 MJCF loader.
// - Native equality/connect site1/site2 support for MuJoCo site-pair loop closures.
...
class MJCFModel extends THREE.Group {
  constructor(name = 'AutoMindMJCF') {
    ...
    // Named MJCF sites are kinematic frames, not merely renderer annotations.
    // equality/connect may reference them through site1/site2 instead of body1/body2.
    this.sites = {};
    this.joints = {};
    this.loopJoints = [];
    this.couplings = [];
...
function registerSite(node, bodyName, model) {
  const name = String(node?.getAttribute?.('name') || '').trim();
  if (!name || !bodyName || !model?._linkInfo?.[bodyName]) return null;
  // MJCF site pose is authored in the owning body's moving coordinate frame.
  // parseQuat returns MJCF w,x,y,z; Three.js frameMatrix expects x,y,z,w.
  const pos = parseVec(node.getAttribute('pos'), [0, 0, 0]);
  const q = parseQuat(node.getAttribute('quat'));
  const localFrame = frameMatrix(pos, [q[1], q[2], q[3], q[0]]);
  const site = {
    name,
    body: bodyName,
    pos: pos.slice(),
...
async function parseBody(node, parentContent, model, assets, raw, db, parentName, serial) {
  ...
  // Register named sites before equality parsing. These frames live after the
  // body's scalar-joint chain, exactly like geoms and child bodies.
  for (const site of childrenByName(node, 'site')) registerSite(site, name, model);
...
function parseEquality(root, model) {
  ...
  for (const n of Array.from(e?.children || [])) {
    ...
    // MuJoCo supports both body-pair and site-pair equality declarations. A
    // site pair is not optional metadata: it supplies the exact local frame for
    // each side of the closure. Previous viewer builds ignored site1/site2,
    // silently dropping valid four-bar loops from BUILD180 MJCF files.
    const site1Name = String(n.getAttribute('site1') || '').trim();
    const site2Name = String(n.getAttribute('site2') || '').trim();
    const siteA = site1Name ? model.sites?.[site1Name] : null;
    const siteB = site2Name ? model.sites?.[site2Name] : null;
    const usesSitePair = !!(site1Name || site2Name);

    if (usesSitePair && (!siteA || !siteB)) {
      model.solverHints.push({ name, type: tag, reason: `Missing equality site pair: ${site1Name || '(none)'} / ${site2Name || '(none)'}` });
      continue;
    }
    ...
    // An omitted body2 is the MuJoCo world body. Site owners override body attrs.
    const body0 = usesSitePair ? siteA.body : (n.getAttribute('body1') || '');
    const body1 = usesSitePair ? siteB.body : (n.getAttribute('body2') || '__world__');
    ...
    // A collinear second connect is the axis point of a revolute loop. Keep it
    // active for DLS closure, but do not render it as a second floating loop.
    const secondaryAxisConnect = /^connect$/i.test(tag) && /(?:^|_)(?:axis_[xyz]|axis_z)(?:$|_)/i.test(name);
    model.loopJoints.push({
      name,
      type: tag,
      role: 'loop',
      tree: false,
      drawable: !secondaryAxisConnect,
      ...
      body0,
      body1,
      site1: usesSitePair ? site1Name : '',
      site2: usesSitePair ? site2Name : '',
      siteFrame0: usesSitePair ? siteA.localFrame.clone() : null,
      siteFrame1: usesSitePair ? siteB.localFrame.clone() : null,
      ...
      // same attribute is relative to body2 (our body1). Site pairs bypass this
      // anchor convention because each site already defines its own body frame.
      anchor,
      hasAnchor: n.hasAttribute('anchor') || usesSitePair,
      relpose,
...
function finalizeLoopAnchors(model) {
  ...
  for (const loop of model.loopJoints || []) {
    ...
    if (/^connect$/i.test(loop.type || '')) {
      // site1/site2 is the authoritative MuJoCo form: each site supplies a
      // local frame in its own moving body. Do not reinterpret it as a body1
      // anchor, and do not derive one side from the reference pose.
      if (loop.siteFrame0?.isMatrix4 && loop.siteFrame1?.isMatrix4) {
        storeLoopFrame(loop, 0, loop.siteFrame0);
        storeLoopFrame(loop, 1, loop.siteFrame1);
        loop.anchorFrame = 'site_pair';
        continue;
      }
...
export async function loadMJCFModel(opts = {}) {
  ...
  model.userData.__automindBuild = 'MJCF BUILD186 — site-pair equality/connect loops';
  ...
  // World-owned named sites are valid equality endpoints too.
  for (const site of childrenByName(worldbody, 'site')) registerSite(site, '__world__', model);
