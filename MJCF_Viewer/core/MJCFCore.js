...
async function textureFromDataUrl(dataUrl, { flipY = true } = {}) {
  ...
  return await new Promise((resolve) => {
    ...
    const finish = (ok) => {
      ...
      const flatColor = inspectFlatTextureColor(image);
      if (flatColor) {
        tex.userData = tex.userData || {};
        tex.userData.__automindFlatTextureColor = flatColor;
      }
...
async function makeMaterial(def, assetDB, geomNode = null) {
  ...
  if (map) {
    ...
    const flat = map.userData?.__automindFlatTextureColor || null;
    const usesSolidTextureColor = Array.isArray(flat) && flat.length >= 3;
    // If a tiny texture cannot be sampled by the browser canvas, MJCF material
    // rgba is still a valid authored colour. Keep the model visible instead of
    // risking a black GPU texture path; detailed texture maps remain unchanged.
    const swatchFallback = !usesSolidTextureColor && /\.(png|jpe?g|webp)$/i.test(requestedTexture) && !!def?.rgba;
    const solidColor = usesSolidTextureColor
      ? new THREE.Color(flat[0], flat[1], flat[2])
      : (swatchFallback ? new THREE.Color(rgba[0], rgba[1], rgba[2]) : new THREE.Color(0xffffff));
    const alpha = usesSolidTextureColor ? ((flat[3] ?? 1) * rgba[3]) : rgba[3];

    // This matches the successful USD/URDF viewport policy: texture-painted CAD
    // surfaces are rendered unlit in the inspection view. Uniform 32x32 colour
    // swatches are promoted to their exact colour, so an OBJ/PNG sampler issue
    // can never turn an otherwise coloured CAD part into a black silhouette.
    mat = new THREE.MeshBasicMaterial({
      color: solidColor,
      map: (usesSolidTextureColor || swatchFallback) ? null : map,
    ...
    mat.userData = {
      ...
      __automindSolidTextureColor: usesSolidTextureColor ? flat.slice() : null,
...
export async function loadMJCFModel(opts = {}) {
  ...
  model.userData.__automindBuild = 'MJCF BUILD181 — post-joint kinematic frames';
