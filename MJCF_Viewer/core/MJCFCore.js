...
async function makeMaterial(def, assetDB) {
  ...
  const mat = new THREE.MeshStandardMaterial({
    ...
    transparent: rgba[3] < 0.999, opacity: rgba[3], side: THREE.DoubleSide
  });
  const tex = def?.texture ? assetDataUrl(assetDB, def.textureCandidates || []) : '';
  if (tex) {
    const map = await textureFromDataUrl(tex);
    if (map) {
      // MJCF material rgba is a multiplier. A CAD color texture should not be
      // multiplied by the same color again; let the texture drive diffuse color.
