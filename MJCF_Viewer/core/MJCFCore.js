...
async function makeMaterial(def, assetDB, geomNode = null) {
  ...
  if (map) {
    ...
    // A texture that decodes as almost entirely black is normally the failed
    // data-URL/UV upload path, not the intended CAD colour. Prefer MTL/MJCF Kd
    // in that case. If Kd is also black, use a neutral inspection fallback only
    // when opts explicitly requested forceVisibleMaterials.
    ...
    const emergencyColor = rgbaLuma > 0.035 ? rgbaColor : new THREE.Color(0xdfe8ea);
    ...
    const solidColor = usesSolidTextureColor
      ...
      : ((swatchFallback || useEmergencyFallback) ? emergencyColor : new THREE.Color(0xffffff));
    const alpha = usesSolidTextureColor ? ((flat[3] ?? 1) * rgba[3]) : rgba[3];

    mat = new THREE.MeshBasicMaterial({
      color: solidColor,
      map: (usesSolidTextureColor || swatchFallback || useEmergencyFallback) ? null : map,
...
export async function loadMJCFModel(opts = {}) {
  ...
  model.applyPose();
  try {
    const report = { meshes: 0, textured: 0, canvasBacked: 0, blackFallbacks: 0, unresolved: [] };
    model.traverse?.((o) => {
      if (!o?.isMesh) return;
      report.meshes++;
