...
export function createToolsDock(app, theme) {
  ...
  // Keep the UI and core mode deterministic even when a previous iframe/output
  // had selected Ghost. The caller can still change it immediately afterward.
  try { setRenderMode('Solid'); } catch (_) {}

  return { open: openDock, close: closeDock, set, destroy, setRenderMode };
}
