...
function nearestOwningLink(node) {
  let p = node;
  while (p) {
    if (p.userData?.__linkName) return p;
    p = p.parent;
  }
  return null;
}
function collectMeshesInLink(linkObj) {
  const out = [];
  // A MJCF body is nested below its parent body. A generic traverse therefore
  // used to collect every descendant link, causing the cyan Box3Helper to form
  // a giant cage around a whole four-bar mechanism. Keep only meshes whose
  // nearest named body is exactly the requested link.
  linkObj?.traverse?.(o => {
    if (!o?.isMesh || !o.geometry || o.userData.__isHoverOverlay) return;
    if (nearestOwningLink(o) === linkObj) out.push(o);
  });
  return out;
}
...
export function attachInteraction({ scene, camera, renderer, controls, robot, selectMode = 'link', getSectionPlane = null, onSelectLink = null }) {
  ...
  function endJointDrag(ev) {
    ...
    controls.enabled = true;
    renderer.domElement.style.cursor = 'auto';
    // A drag is a motion command, not a persistent component selection. Remove
    // the cyan bounding cage after an actual joint move; a click-without-motion
    // still retains normal selection semantics.
    if (pending?.moved) clearSelection();
    else refreshSelectionMarker();
