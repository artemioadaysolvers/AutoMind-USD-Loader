...
function createViewportTextureProxy(core, robot, enabled = true) {
  // Colab's visible default framebuffer can, on some GPUs, show a black output
  // for textured OBJ materials even though the exact same map is valid in a
  // WebGLRenderTarget (the component thumbnails).  This presentation bridge
  // renders a small scene-level copy of each visual mesh with the same proven
  // MeshBasicMaterial+map path as the thumbnail renderer.  The original meshes
  // remain visible to the raycaster and keep all joints / selection behavior;
  // they simply stop writing color/depth while the proxy paints the viewport.
  ...
  robot.traverse?.((source) => {
    ...
    const proxyMats = sourceMats.map((src) => {
      ...
      } else if (src?.color?.clone) {
        fallbackProxyMeshes++;
        color = src.color.clone();
        // Never allow an unresolved black source fallback to recreate the
        // original black silhouette.  The MTL bridge already uses this neutral
        // inspection color for genuinely missing images.
        if (color.r < 0.03 && color.g < 0.03 && color.b < 0.03) color.setHex(0xb8c8cf);
      } else {
        fallbackProxyMeshes++;
        color = new THREE.Color(0xb8c8cf);
      }
      ...
      pm.userData = {
        ...(pm.userData || {}),
        __automindViewportProxyMaterial: true,
        __automindTextureMap: map || null,
      };
      if (map) {
        try {
    ...
    const proxy = new THREE.Mesh(source.geometry, Array.isArray(source.material) ? proxyMats : proxyMats[0]);
    ...
    for (const mat of sourceMats) {
      ...
      // Original mesh stays visible for interaction/raycasting but contributes no
      // framebuffer color or depth. Hover overlays are different meshes and are
      // intentionally left untouched.
      mat.colorWrite = false;
      mat.depthWrite = false;
      mat.needsUpdate = true;
    }
    records.push({ source, proxy, sourceStates, proxyMats });
  });
  ...
  const sync = () => {
    ...
    for (const record of records) {
      ...
      for (let i = 0; i < proxyMats.length; i++) {
        ...
        const map = src.map || srcUd.__automindTextureMap || null;
        if (map && pm.map !== map) pm.map = map;
        if (pm.map) {
          try { pm.map.needsUpdate = true; } catch (_) {}
          pm.color.setHex(0xffffff);
        } else if (src.color?.copy) {
          pm.color.copy(src.color);
          if (pm.color.r < 0.03 && pm.color.g < 0.03 && pm.color.b < 0.03) pm.color.setHex(0xb8c8cf);
        }
  ...
  const report = {
    ...
    mappedProxyMeshes,
    fallbackProxyMeshes,
    note: 'Viewport proxy uses the same map path proven by thumbnail render targets.'
  };
...
export function render(opts = {}) {
  ...
  app.ready = (async () => {
    try {
      ...
      try { createViewportTextureProxy(core, robot, opts.viewportTextureProxy !== false); } catch (proxyError) {
