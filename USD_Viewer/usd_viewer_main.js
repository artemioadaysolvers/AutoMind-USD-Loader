...
function tweenCamera(core, endPos, endTarget, duration = 750) {
  ...
  function step(now) {
    ...
    else if (token === cameraTweenToken) ctrl.enabled = true;
  }
...
function targetVisibleOpacityFor(mat, meshWasVisible) {
  ...
  // Important for rapid clicks: if a fade-out was interrupted, currentOpacity can
  // be mid-way (ex: 0.23). Fade-in must return to the last real visible opacity,
  // not stop at that half-transparent value.
  if (Number.isFinite(ud.__automindVisibleOpacity) && ud.__automindVisibleOpacity > 0) {
    return Math.max(current, Math.max(0, ud.__automindVisibleOpacity));
  }
  if (current > 0) return current;
  return meshWasVisible ? Math.max(current, 1) : 1;
}
